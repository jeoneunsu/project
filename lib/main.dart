...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      // 앱의 기본적인 테마를 지정
      theme: ThemeData(
          primarySwatch: Colors.grey // primarySwatch 기본적인 앱의 색상을 지정
      ),
...
class _TabPageState extends State<TabPage> {
  ...
  // 이동할 페이지
  List _pages = [Home(), ExList(), Text('page3'), Board(), Mypage()];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        bottomNavigationBar: BottomNavigationBar(
          type: BottomNavigationBarType.fixed, // 메뉴 세개 ?이상일 때 무조건 넣어주기
          showSelectedLabels: false,
          showUnselectedLabels: false, // 라벨 없애기
          ...
          items: <BottomNavigationBarItem>[
            ...
            BottomNavigationBarItem(
                icon: Icon(Icons.book), label: 'Exhibition'),

            BottomNavigationBarItem(
                icon: Icon(Icons.location_on), label: 'Map'),

            BottomNavigationBarItem(
                icon: Icon(Icons.chat), label: 'Board'),
            BottomNavigationBarItem(
                icon: Icon(Icons.people), label: 'Mypage'),
